...
Many people who promote R quote the number of R blogs as given on the R-Bloggers
website, which syndicates literally hundreds of R-related blogs.  But the number
tends only to increase.  How many actual posts are there in a given week/month,
from how many different blogs?

## The gist of it

I subscribed to the R-Bloggers daily digest emails in late 2014, giving me a
good time-series of posts.
...
It turns out that there are about XXX blogs active in a given month, posting
YYY posts (Revolutions is the only one that regularly posts more than once per
week).  Nothing much has changed in the last year.  For some
arbitrary definition of "dead blog", a survival analysis could be done.
...
convert_mbox_eml("./data/clean.mbox", mail_dir)
...
blogs %>%
  ...
  coord_flip() +
  xlab("") +
  ylab("Number of posts since November 2014") +
  ggtitle("Top-10 bloggers by number of posts since November 2014") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

```{r rbloggers-rates, echo = FALSE}
# Rates of blogging (smoothed difference in days)
logweek_trans <- function() {log_trans(9)}
blogs %>%
  count(blog, datetime) %>%
  group_by(blog) %>%
  arrange(datetime) %>% 
