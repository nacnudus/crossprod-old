...
```{r brexit-boilerplate, include = FALSE}
...
master_ft %>% 
  ...
  geom_point(aes(y = undecided, shape = sample_size), stroke = 0.8) +  
...
master_ft %>%
  ...
  geom_bar() +
  annotate("text", "leave",  4, label = "Leave") +
  annotate("text", "remain",  4, label = "Remain") +
  xlab("") +
...
master_ft %>%
  ...
  geom_point(aes(colour = majority, shape = sample_size), stroke = 0.8) +  
