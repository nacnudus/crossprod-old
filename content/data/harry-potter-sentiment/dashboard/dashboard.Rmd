...
novels.  Here, it is applied to the Harry Potter series.
...
renderPlot({
  ngrams() %>%
    ...
    ggplot(aes(ngram, n)) +
    coord_flip() +
    geom_text(aes(label = ngram), hjust = 0, size = input$textsize) + 
    scale_y_continuous(limits = c(0, maxcount() * 1.25), expand = c(0, 0)) +
    xlab("") +
    theme_minimal() +
