...
Summary: Reproducing browser rounding in R
...
A great R-newbie gotcha is rounding-to-even (example adapted from ?round):
...
This post draws illustrates rounding bias with a graph, discovers that
JavaScript (in my browser) rounds the other way, encounters floating-point
difficulties when emulating JavaScript, and finally calls JavaScript itself via
the V8 package.
...
## Calling JavaScript V8 engine via V8 package
...
```{r, ref.label = 'rounding-bias-1', echo = TRUE, eval = FALSE}
```

```{r, ref.label = 'rounding-bias-2', echo = TRUE, eval = FALSE}
```
