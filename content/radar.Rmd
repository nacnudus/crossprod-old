...
Summary: Get a perfect score in the Mango Solution Data Science Radar
...
Why hack?  Because getting at the innards would also reveal
...
![Mango Solutions Data Science Radar](figure/radar.png)
...
so instead, let's use science: hypothesise -> text -> improve.  Here are some
initial guesses.
...
random_data <- read_csv("./data/radar-random.csv")
...
```{r radar-systematic}
systematic_data <- read_csv("./data/radar-systematic.csv")
systematic <- 
  systematic_data %>%
...
```{r radar-perfect}
perfect_data <- read_csv("./data/radar-perfect.csv")
systematic <- 
  perfect_data %>%
  select(-(starts_with("q"))) %>%
  mutate(statement = 1:n() - 1) %>%
...
to "Strongly agree".  Note the change of x-axis variable.

```{r radar-diff}
penalty <-  "./data/radar-penalties.csv" %>% read_csv
penaltygraph <- 
  penalty %>%
  select(-1:-24) %>%
...
So penalties can be as large as 1.8.  That would be problem for poor-ranking
...
![Mango Solutions Data Science Radar: worst score](figure/radar-worst.png)
...
[previous post]({filename}./rounding.Rmd).  At least I got to display some
'Technologist' skills.
...
questions <- read_csv("./data/radar-questions.csv")
...
# # posterity
# results %>% write_csv("./data/radar-results.csv")
results <- read_csv("./data/radar-results.csv")
...
* 5.2 > 7.0 for unicorns with perfect answers.
...
![Mango Solutions Data Science Radar: perfect score](figure/radar-perfect.png)
